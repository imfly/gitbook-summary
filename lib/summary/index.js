...
function Summary(options) {
  ...
  async.auto({
    ...
    files: ['init', function(next) {
      getAllFiles(root, next(null, filesObj));
    //   next();
    }],

    parse: ['files', function(filesObj, next) {
      work(filesObj);
      next();
    }],
...
function getAllFiles(bookRoot, next) {
  var result = {},
    filesJson = {};

//    fixme to use Async for reading the files?
   files(bookRoot, filesJson, function(err, json) {
